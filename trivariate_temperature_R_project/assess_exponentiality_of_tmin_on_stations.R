...
# Load data ---------------------------------------------------------------
ghcnd_stations_tmin_97p_hhy_events_disagg_df <- read_csv("../data/ghcnd_tmin_97p_hhy_events_disagg.csv")
...
N = 50
stations_of_interest <- (ghcnd_stations_tmin_97p_hhy_params_df %>%
                           filter(n_events >= 100) %>%
                           slice_sample(n = N))$unique_id
for(i in seq_along(stations_of_interest)) {
  cur_station = stations_of_interest[i]
  png(filename = paste0("../images/ghcnd/goodness-of-fit/exponentiality-qq-plots/",
                        cur_station,
                        "qq_plot.png"))
  qqmath(~exceedance,
         data = ghcnd_stations_tmin_97p_hhy_events_disagg_df %>%
           filter(station == cur_station),
         distribution = function(p) {qexp(p, rate = (ghcnd_stations_tmin_97p_hhy_params_df %>%
